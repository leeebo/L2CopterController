...
volatile uint8_t Autofly_toggle=0;
volatile uint8_t Autofly_unlock=0;
volatile uint8_t Autofly_ing=0;
volatile uint8_t Autofly_unlock_ing=0;

volatile uint8_t vision_lock=0;
volatile uint8_t if_vision_lock=0;
volatile uint8_t vision_ing=0;
// -------------------------------------------------------------
// FAILSAFE MODE
// -------------------------------------------------------------
...
// Throttle auto fly
#define PPM_THROTTLE_AUTOFLY  ONE_US * 1680 - PPM_PRE_PULSE
...
#define PPM_CH5_MODE_3        ONE_US * 1400 - PPM_PRE_PULSE
...
volatile uint16_t ppm_auto[ PPM_ARRAY_MAX ] =
{
	PPM_PRE_PULSE,
	PPM_SERVO_CENTER,         // Channel 1
	PPM_PRE_PULSE,
	PPM_SERVO_CENTER,         // Channel 2
	PPM_PRE_PULSE,
	PPM_THROTTLE_AUTOFLY,     // Channel 3 (throttle)
	PPM_PRE_PULSE,
	PPM_SERVO_CENTER,         // Channel 4
	PPM_PRE_PULSE,
	PPM_CH5_MODE_3,           // Channel 5
...
volatile uint16_t ppm_auto_throttle=ONE_US * 1000 - PPM_PRE_PULSE;
volatile uint16_t vision_ing_ch1=ONE_US * 1500 - PPM_PRE_PULSE;
volatile uint16_t vision_ing_ch2=ONE_US * 1500 - PPM_PRE_PULSE;
volatile uint16_t vision_ing_ch7_land=ONE_US * 1950 - PPM_PRE_PULSE;
volatile uint8_t  vision_land_flag=0;
// -------------------------------------------------------------
...
CHECK_PINS_LOOP: // Input servo pin check loop
    ...
    if( servo_change & servo_pin )
    {
        ...
        else
        {
		    ...
		    if (ppm_channel==13&&vision_lock==0)
		    {
				if (servo_width>(ONE_US * 1200 - PPM_PRE_PULSE))
				{
					//vision_lock=1;
					Autofly_ing=4;
				}
			...
			else if (ppm_channel==1)
			{
				if (servo_width<(ONE_US * 1550 - PPM_PRE_PULSE)&&servo_width>(ONE_US * 1450 - PPM_PRE_PULSE))
				{
					if_vision_lock=0;
				}
				else 
			...
			else if (ppm_channel==3&&if_vision_lock==0)
			{
				if (servo_width<(ONE_US * 1550 - PPM_PRE_PULSE)&&servo_width>(ONE_US * 1450 - PPM_PRE_PULSE))
				{
					vision_lock=0;
			...
			if (vision_lock==0)
			{
						    if (ppm_channel == 15)
						    {
							    if (servo_width>(ONE_US * 1200 - PPM_PRE_PULSE)&&servo_width<(ONE_US * 1900 - PPM_PRE_PULSE))
							    {
								    vision_ing=1;
									if (servo_width>(ONE_US * 1250 - PPM_PRE_PULSE)&&servo_width<(ONE_US * 1350 - PPM_PRE_PULSE))
									{//左
										vision_ing_ch1=ONE_US * 1150 - PPM_PRE_PULSE;
										vision_ing_ch2=ONE_US * 1500 - PPM_PRE_PULSE;
									}
									else if (servo_width>(ONE_US * 1650 - PPM_PRE_PULSE)&&servo_width<(ONE_US * 1750 - PPM_PRE_PULSE))
									{//右
											vision_ing_ch1=ONE_US * 1850 - PPM_PRE_PULSE;
											vision_ing_ch2=ONE_US * 1500 - PPM_PRE_PULSE;
									}
									else if (servo_width>(ONE_US * 1350 - PPM_PRE_PULSE)&&servo_width<(ONE_US * 1450 - PPM_PRE_PULSE))
									{//后
											vision_ing_ch1=ONE_US * 1500 - PPM_PRE_PULSE;
											vision_ing_ch2=ONE_US * 1850 - PPM_PRE_PULSE;
									}
									else if (servo_width>(ONE_US * 1550 - PPM_PRE_PULSE)&&servo_width<(ONE_US * 1650 - PPM_PRE_PULSE))
									{//前
											vision_ing_ch1=ONE_US * 1500 - PPM_PRE_PULSE;
											vision_ing_ch2=ONE_US * 1150 - PPM_PRE_PULSE;
									}
									else if (servo_width>(ONE_US * 1750 - PPM_PRE_PULSE)&&servo_width<(ONE_US * 1850 - PPM_PRE_PULSE))
									{//前
										    vision_land_flag=1;
									}
								    else
            ...
            if (ppm_channel == 11)//第6通道
            {
				if (servo_width>(ONE_US * 1700 - PPM_PRE_PULSE))
				{
					Autofly_toggle=1;
					Autofly_unlock=1;
					if (Autofly_unlock_ing==0)
					{
						Autofly_unlock_ing=1;
					}
					if (Autofly_unlock_ing==3)
					{
						if (Autofly_ing==0)
					  {
						Autofly_ing=1;
...
ISR( PPM_INT_VECTOR, ISR_NOBLOCK )  
{
    ...
    else
    {
        ...
        cli();
		if (Autofly_unlock==1&&Autofly_unlock_ing==1)
		{
			if (ppm_out_channel==7)
			{
				PPM_COMPARE +=ONE_US * 1998 - PPM_PRE_PULSE;
			}
			else PPM_COMPARE += ppm[ ppm_out_channel ];
			sei();
		...
		else if(Autofly_toggle==1&&Autofly_ing==1&&Autofly_unlock_ing==3)  //赋值为自动起飞模式
		{
			        if (ppm_out_channel==5)
			        {
					   PPM_COMPARE +=ppm_auto_throttle;	
			        }
			        else PPM_COMPARE += ppm[ ppm_out_channel ];
			        sei();
		...
		else if (Autofly_toggle==1&&Autofly_ing==2&&Autofly_unlock_ing==3)
		{
						        if (ppm_out_channel==5)
						        {
							        PPM_COMPARE +=ONE_US * 1500 - PPM_PRE_PULSE;
						        }
								else if (vision_ing==1&&vision_lock==0)
								{
									      if (ppm_out_channel==1&&vision_land_flag==0)
									      {
										      PPM_COMPARE +=vision_ing_ch1;
									      }
									      else if (ppm_out_channel==3&&vision_land_flag==0)
									      {
										      PPM_COMPARE +=vision_ing_ch2;
									  	  }
											 else if (ppm_out_channel==13&&vision_land_flag==1)
											{
												PPM_COMPARE +=vision_ing_ch7_land;
											}
									      else PPM_COMPARE += ppm[ ppm_out_channel ];
						        ...
						        sei();
								if (ppm[5]>(ONE_US * 1400 - PPM_PRE_PULSE)&&ppm[5]<(ONE_US * 1550 - PPM_PRE_PULSE))
								{
									Autofly_ing=3;
								}
			        ...
			        if (vision_ing==1&&vision_lock==0&&Autofly_ing==3)
					{
						    //if (ppm_out_channel==1)
						    //{
							    //PPM_COMPARE +=vision_ing_ch1;
						    //}
							//else if (ppm_out_channel==3)
							//{
								//PPM_COMPARE +=vision_ing_ch2;
							//}
							 if (ppm_out_channel==1&&vision_land_flag==0)
