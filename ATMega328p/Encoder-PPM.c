...
int main(void)
{
	...
	static uint16_t autofly_timer=0;
	static uint16_t autofly_unlock_timer=0;
	...
	while( 1 )
	{
		...
		else		// We are running normally
		{
			if ( mux_passthrough == false ) //  Normal mode : status LED toggle speed from throttle position
			{
				...
				if (Autofly_ing==1)
				{
				    led_acceleration=(ppm_auto_throttle-( PPM_SERVO_MIN ) ) / 2;
					if (autofly_timer<3000)
					{
						PPM_PORT |= ( 1 << PB0 );  // Enable status LED
					}
					else blink_led (LED_LOW_BLINKING_RATE);
				}
				else blink_led ( LED_LOW_BLINKING_RATE - led_acceleration );
			}
		...
		{
			autofly_unlock_timer++;
			if (autofly_unlock_timer>3500)
			{
				
				autofly_unlock_timer=0;
				Autofly_unlock_ing=2;
				
			}
			
		}
		else if (Autofly_unlock==1&&Autofly_unlock_ing==2)
		{
			autofly_unlock_timer++;
			if (autofly_unlock_timer>3000)
			{
				
				autofly_unlock_timer=0;
				Autofly_unlock_ing=3;
		...
		//添加自动起飞计时代码
		if (Autofly_toggle==1&&Autofly_ing==1&&Autofly_unlock_ing==3)
		{
			autofly_timer++;
			if (autofly_timer<4000)
			{
				ppm_auto_throttle=ppm_auto_throttle+ONE_US; //1000到1500 1.5秒
				if (ppm_auto_throttle>(ONE_US * 1500 - PPM_PRE_PULSE))
				{
				    ppm_auto_throttle=(ONE_US * 1500 - PPM_PRE_PULSE);
				}
				
			}
			else if (autofly_timer<6000)
			{
			    ppm_auto_throttle=ppm_auto_throttle+ONE_US;//1680 1秒
				if (ppm_auto_throttle>(ONE_US * 1690 - PPM_PRE_PULSE))
				{
					ppm_auto_throttle=(ONE_US * 1690 - PPM_PRE_PULSE);
				}
			}
			else if (autofly_timer>6000)
			{
				 ppm_auto_throttle=ppm_auto_throttle-ONE_US;
				 if (ppm_auto_throttle<(ONE_US * 1500 - PPM_PRE_PULSE))
				 {
					 ppm_auto_throttle=(ONE_US * 1500 - PPM_PRE_PULSE);
					 autofly_timer=0;
		...
		{
			if ( servo_error_detection_timer > ( ERROR_DETECTION_WINDOW ) )	// If 30s delay reached
			{
		...
		{	
			...
			if ( servo_error_condition_timer > ( ERROR_CONDITION_DELAY ) )	// If 5s delay reached
			{
				servo_error_condition_timer = 0;	 						// Reset servo error condition timer
		...
		_delay_us (950); // Slow down while loop950改940
